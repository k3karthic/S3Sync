...
import s3sync_lib as lib

def list_files(config,files):
...
def calc_diff(config):
    ...
    with codecs.open(s3_list_file,encoding='UTF-8',mode='r') as sfile:
        for line in sfile:
            ...
            s3key_hash = lib.create_digest(s3key)
    ...
    with codecs.open(local_list_file,encoding='UTF-8',mode='r') as lfile:
        for line in lfile:
            ...
            file_key = lib.convert_to_s3key(file_name,dir_name,alias)
            file_key_hash = lib.create_digest(file_key)
    ...
    with codecs.open(last_sync_file,encoding='UTF-8',mode='r') as sfile:
        for line in sfile:
            ...
            key_hash = lib.create_digest(key)
